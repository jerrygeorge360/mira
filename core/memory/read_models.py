...
def list_foresight_read_model(
    ...
    query = "SELECT * FROM foresight_records WHERE status = ? ORDER BY created_at DESC LIMIT ?"
    params: tuple[object, ...] = (status, limit)
    if session_id:
        query = """
        ...
        LIMIT ?
        """
        params = (status, session_id, limit)
    return _fetch_all(query, params)


def list_reflections_read_model(limit: int = 50) -> list[MemoryRecord]:
    """Return reflection records for read-only presentation."""
    return _fetch_all("SELECT * FROM reflections ORDER BY created_at DESC LIMIT ?", (limit,))


def list_community_summaries_read_model(limit: int = 50) -> list[MemoryRecord]:
    """Return community summaries for read-only presentation."""
    return _fetch_all(
        "SELECT * FROM community_summaries ORDER BY created_at DESC LIMIT ?",
        (limit,),
    )
