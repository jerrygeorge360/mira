...
- [~] **D3. Re-run** (partial, DeepSeek + local embeddings). Local eval **11/13** (0.85): 10 original PASS; new D2 cases — community PASS, and two revealing FAILs: `contradiction-deadline-conflict` created `SUPERSEDED_BY` not `CONTRADICTS` (canonicalization makes the pair share a canonical subject+predicate → deterministic fast-path claims it as an update before the LLM verifier; answer still surfaced both + flagged — open `[decide]`), `reflection-self-knowledge` retrieved community_summary+observation but **no reflection** in Deep (fire-vs-retrieve, needs `--debug-trace`). Ablation **incomplete + mixed** (5/11 configs, inconsistent case counts from resuming separately-scoped runs) — rerun clean in one invocation. `[verify]`.
